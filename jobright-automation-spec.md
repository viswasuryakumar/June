...
> **Open implementation gap (REV-001):** `src/auth/check.py` does not currently exist, so the command named by the DoD below cannot run and the DoD is not implementation-complete.
